...
| **`git push` rifiutato: «Push cannot contain secrets»** | è la *push protection* di GitHub, e sul repo di una skill che caccia segreti scatta sulle **esche dei test** — che devono avere la forma vera, o il test non proverebbe niente. Nei file nuovi le esche si **compongono** (`["sk","live",…].join("_")`): la stringa che arriva al codice è identica, ma non esiste come sequenza nel file. Per le esche già nei commit passati non c'è scorciatoia onesta: o si dichiara il falso positivo dalla pagina che GitHub stampa nell'errore (serve il proprietario del repo), o si riscrive la storia — che per una stringa finta costa più del difetto, e invalida gli hash che `ARCHIVIO.md` cita |
