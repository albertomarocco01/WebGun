...
| D8 | **Parallelismo a file disgiunti.** Possono correre insieme solo pacchetti i cui perimetri di **scrittura** non si intersecano e che non condividono un banco. Ogni operaio committa **solo i propri percorsi** (`git add` espliciti, mai `-A`, mai `commit -a`: l'index git è unico) e **non tocca `CANTIERE.md`** — la contabilità la scrive il direttore ai ritorni. Assetto corrente: **P.2-ripresa ∥ P.7a ∥ P.7b**; **P.7c in coda a P.2** (banco vetcare e Docker condivisi) | richiesta di velocità del committente (2026-08-03); il working tree e l'index sono unici, e due gate sullo stesso banco producono misure sporche (lezione D6) | presa 2026-08-03 |
...
| P.7a | Gate della regia | lo dichiara mancante `DECISIONI.md` §26: controllo docx/txt allineati, più le coerenze della regia (junction, elenchi README vs skill reali) | — | script deterministico, verde riproducibile da un clone pulito; regole in lib pura con test; guardiani locali per `scripts/` di radice | **in corso** — mandato emesso 2026-08-03 (`prompts/P7a-gate-regia.md`, Opus 5 · high) |
| P.7b | Documento madre aggiornato | `Web Gun.docx` fermo (schema-forge dichiarato a versione vecchia) + `scripts/estrai-docx.ps1` rilanciato | — (il .docx lo edita Alberto in Word, con STOP a metà mandato) | `webgun_content.txt` rigenerato e coerente col repo | **in corso** — mandato emesso 2026-08-03 (`prompts/P7b-docx.md`, Sonnet 5 · high) |
...
- **2026-08-03 (sera, pit stop)** — **P.8 collaudata dal direttore**, tutto rilanciato in
