...
| ~~**quello che invece e' pubblico davvero**~~ **NON E' PIU' CIECA (collaudo P2, 2026-08-04)** | ~~la chiave anonima sta nel bundle, e PostgREST accetta `?select=` da chiunque~~ | **misurato in P1**: `curl "…/rest/v1/corsi?select=id,created_at,in_evidenza" -H "apikey: <anon>"` risponde con quelle colonne, che nessuna pagina seleziona e nessuna disegna. **Cio' che e' pubblico lo decide il `grant` piu' la policy, non l'elenco del `select`.** **In P2 questa riga e' diventata una regola**: `contenuti-vivi` confronta le colonne firmate in §Dati visibili a un anonimo con quelle su cui `anon` ha `select` (`information_schema.column_privileges`), e ogni colonna concessa e non firmata e' un `block`. Sul banco `banco-prova-valscura`: **22 firmate, 36 concesse, 14 di differenza**, e il gate era verde. La classe era dichiarata cieca perche' nessuno aveva provato a guardarla, non perche' non si potesse |
| ~~dichiara `Nessuno slot.` su un sito coi testi cablati~~ **QUASI PIU' CIECA (collaudo P2, 2026-08-04)** | ~~e' una dichiarazione firmata, non una misura~~ | **verde 10/10 in P1** col testo di `corsi-intro` cablato nel JSX e `Nessuno slot.` nel contratto. Riprovata in P2 nella forma piu' subdola — non `Nessuno slot.` ma **una sola riga tolta** dal contratto, coi suoi 331 caratteri cablati nel JSX: **verde 10/10**, perche' il ciclo dei controlli girava su `contratto.slot` e togliere una riga ne portava fuori dal perimetro anche il difetto. Ora la regola 2 si applica anche alle righe PUBBLICATE che nessuno slot dichiara — il database e' un elenco che nessuno accorcia riscrivendo un documento — e la testa del passo stampa quante sono anche sul verde. **Resta cieco** il solo caso in cui il testo cablato nel database non esiste affatto: li' non c'e' niente con cui confrontarlo |
| ~~aggiungi una rotta servita da un `route.ts`~~ **NON E' PIU' CIECA (collaudo P2, 2026-08-04)** | ~~la seconda direzione di `pagine-vive` enumera i `page.tsx`, non l'app~~ | **verde in P1**: `/api/iscrizioni` rispondeva `{"aperte":false}` e il gate contava `5 rotte pubbliche nei sorgenti`. Riprovata in P2 su `banco-prova-valscura` (`/disponibilita` che risponde `200` con dei dati): **verde 10/10**, e il test di P1 pretendeva `null` su un `route.ts`, cioe' CODIFICAVA la cecita'. Ora `rottaDaFile` riconosce due foglie (`page.*` e `route.*`), il conteggio e' separato per tipo e un gestore non dichiarato e' un `issue`. Resta invisibile solo cio' che nessun file rappresenta: una riscrittura di `next.config`, una rotta del middleware |
