...
- **Non collauda gli altri passi del gate.** La classe E ne prova alcuni sulle spec (`lint-spec`, `spec-coverage`); `app-viva`, `effetto-db` e `contratto-uscita` hanno i loro test in `scripts/gate-lib.test.mjs` e `scripts/verify.test.mjs`, che è il posto giusto: si eseguono a ogni modifica, senza rompere niente in casa di nessuno.
