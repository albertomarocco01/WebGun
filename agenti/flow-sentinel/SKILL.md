...
- **`verify`** → `node <skill>/scripts/verify.mjs [--json]`: i sette passi della §Gate. All'utente riporto **solo il residuo** e le **verifiche mancanti**, mai i log grezzi.
- **`evolve`** → confronto rotte e handoff nuovi con `docs/flussi-critici.md`: flussi nuovi → Specchio solo su quelli; flussi spariti → la spec diventa orfana e lo segnalo (non la cancello in silenzio: decidere che un flusso non è più critico spetta a chi ha confermato l'elenco).
