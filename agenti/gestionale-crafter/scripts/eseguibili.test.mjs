 ...
 *
 * I test di `scegliEseguibile` e di `formaEseguibile` vengono da
 * `verify.test.mjs` (shim `.cmd` di Windows, 2026-07-28) e ne CORREGGONO due,
 * che asserivano il comportamento poi risultato difettoso:
...
test("una radice admin con lo spazio e' ostile: e' il progetto auditato a scriverla", () => {
  assert.deepEqual(argomentiOstiliACmd(["src/app/mia admin"], "win32"), ["src/app/mia admin"]);
});

test("gli argomenti veri di `npx tsc` e `npx eslint` restano leciti", () => {
  assert.deepEqual(argomentiOstiliACmd(["--no-install", "tsc", "--noEmit"], "win32"), []);
  assert.deepEqual(argomentiOstiliACmd(["--no-install", "eslint", "src/app/admin", "src/components"], "win32"), []);
});

test("fuori da Windows non c'e' cmd, e nessun argomento e' ostile", () => {
  assert.deepEqual(argomentiOstiliACmd(["/&ver", "tsc --noEmit"], "linux"), []);
});
