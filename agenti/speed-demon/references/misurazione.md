...
Il difetto che il secondo comando previene: Lighthouse non porta Chrome con sé, se lo fa trovare. Su una macchina con più binari installati — il canale stabile, il Chromium che Playwright ha scaricato per Flow Sentinel, un residuo di Puppeteer — può partire un Chrome diverso da quello del giro precedente, e la versione di Chrome sposta i numeri per gli stessi motivi per cui li sposta la versione di Lighthouse. Chi misura imposta `CHROME_PATH` e scrive nel metodo quale binario ha misurato. **Il gate non lo verifica**, e questa riga fino al 2026-07-30 dichiarava «o `CHROME_PATH` è impostata, o il passo è `MANCANTE`»: `verify.mjs` non legge mai `process.env.CHROME_PATH` — l'unica occorrenza della stringa nello script è un commento su `--no-sandbox`. Era una regola scritta e non implementata, cioè la forma di falso verde che il collaudo avversario ha trovato sei volte in questa stessa skill. Finché resta così, la versione di Chrome è **una premessa dichiarata dall'operatore**, non un controllo: sta in `STATO.md` fra i punti aperti.

Lighthouse **andrebbe** installato nella cartella della skill e bloccato a una versione esatta, come ESLint per Flow Sentinel e `.sqlfluff` per Schema Forge (`../../../DECISIONI.md` §8): il punteggio di performance è una combinazione pesata di metriche, e i pesi cambiano fra versioni maggiori. Due numeri presi con due versioni diverse non sono confrontabili, e il «guadagno» che ne esce è un cambio di scala travestito da lavoro. **Oggi non è così:** in `agenti/speed-demon/` non ci sono `package.json` né `package-lock.json` — a differenza degli altri tre agenti con gate, che li hanno — e il passo `misura` lancia `npx --yes lighthouse`, cioè prende quello che trova. La versione usata va quindi letta e scritta nel metodo a ogni tornata, e due misure di giorni diversi vanno confrontate solo dopo aver verificato che la versione sia la stessa.
