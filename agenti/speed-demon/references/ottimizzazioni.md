...
| `prefetch` dei `Link` | nessuna metrica di Lighthouse | richieste e carico sul database | nulla — costa al server, non al punteggio |
...
**Cosa può diventare rosso.** Le spec che asseriscono subito dopo il `goto`: il componente arriva con la sua richiesta, e `toBeVisible()` scade prima. Le spec dei flussi ostili in lettura scritte bene diventano invece rosse **al contrario** e per un motivo buono da capire: `references/playwright.md` pretende l'asserzione sul **corpo servito** (`risposta.text()`), quindi un contenuto spostato in `ssr: false` sparisce dal corpo e un'asserzione di *presenza* fallisce, mentre una di *assenza* passa senza aver dimostrato niente. Qui il rosso e il verde si scambiano di posto, ed è il motivo per cui questa voce va dichiarata nel piano parola per parola.
...
| `prefetch` acceso su liste lunghe | nessun flusso: richieste e carico sul database | si vede nei log del server, non nel punteggio | **no** |
