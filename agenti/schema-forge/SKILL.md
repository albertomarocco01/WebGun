...
| File | Cosa |
|---|---|
| `scripts/verify.mjs` | il gate (§Gate di chiusura) — esporta `ID`, `CONTRATTO_JSON`, `conRitentativo`, `dettaglioReset`, `dettaglioAdvisors`, `fileNonLintati`, `formaEseguibile`, `limiteSqlfluff`, `riepilogo`, `schemiEsposti`, `soloSql`, `urlDbProgetto`, `contrattoUscita` |
