...
- **2026-08-06 — P.7d: i rilievi del tribunale che toccano questa skill sono CHIUSI** (verbale con le uscite incollate: `../../PROCESSO-GATE-2026-08-06.md`). Ognuno riprodotto **prima** di correggere e rimisurato dopo. Batteria **156 → 186**, ESLint 0, knip 0, semgrep 0, gitleaks 0, jscpd da 2 cloni (18 righe, 0,51%) a **1 clone** (8 righe, 0,25%).
  - **C1 — CHIUSO.** Quattro chiusure, non una: `where "$PATH:<nome>"`; `where.exe` e `cmd.exe` col percorso pieno; i candidati **dentro** il progetto rifiutati (e il percorso rifiutato stampato); un nome non risolto **non si lancia nudo**. Più `process.execPath` per l'audit RLS. Le regole vivono in `scripts/eseguibili.mjs` con la loro batteria. **Sabotaggio**: prima, i falsi piantati nella radice erano eseguiti 10 volte e portavano a casa `db-reset`, `db-lint` e `db-advisors`; dopo, il file-traccia **non viene creato affatto** e il vero `supabase` dice «supabase start is not running». *Esteso rispetto al referto e misurato*: `spawnSync` col nome nudo risolve dalla directory corrente **da solo**, senza `where` — corrette anche le `spawnSync("psql", …)` di `rls-audit.mjs` ed `erd.mjs`.
  - **H5 + M17 — CHIUSI.** Ognuna delle undici query dichiara la propria arità; un record che non ha i suoi campi **non si interpreta** e l'audit esce 2, cioè MANCANTE. Otto asserzioni sulle forme misurate, fra cui quella che mostra il `block` che sparisce.
  - **M1 — CHIUSO** ai due punti di chiamata riscritti (`rls-audit.mjs`, `erd.mjs`): `-X` è arrivato con la funzione.
  - **M14 + M16 — CHIUSI.** `timeout` più `PGCONNECT_TIMEOUT` su psql; `db reset` con 10 minuti — e lì il limite vale doppio, perché `conRitentativo` **attendeva** il ritorno del primo tentativo e senza limite il ritentativo non partiva. Misurato contro un socket che accetta e non parla: **da ucciso a 100 s (una riga) a 10,3 s col motivo scritto**.
  - **L1 — CHIUSO**: il commento dice ancora perché non si usa `shell: true`, **e** che `cmd.exe /c` una shell lo è lo stesso. Con esso arriva il filtro degli argomenti ostili ai tre punti in cui questa skill passa da uno shim.
  - **Restano aperti** → **tutti chiusi da P.7e il 2026-08-06/07**: M3, M12, M13, M2, L12. Vedi la voce qui sotto.
  - **MANCANTE, con il suo nome**: il gate **non è stato rilanciato contro un banco vivo** — D17 tiene acceso un solo stack, quello del pilota, di P.4h. I rilievi sono stati riprovati per batteria e per sabotaggio su progetti finti; il giro completo è l'atto di chiusura del prossimo pacchetto. Stesso precedente di P.7c del 2026-08-06. **Secondo MANCANTE**: il controllo di arità non è stato esercitato dal guscio — servirebbe un finto `psql.exe`, e uno `.cmd` ora viene respinto prima dal filtro di H1/H2 (respingimento misurato, uscita 2).
...
- **2026-08-06/07 — P.7e: il n°50, cinque voci del referto e due difetti nuovi. Verbale: `../../PROCESSO-GATE-2-2026-08-06.md`.** Batteria **186 → 216**. ESLint 0, knip 0, semgrep 0, gitleaks 0, gate della regia VERDE 5/5.
  - **M12 — CHIUSO, ed è la voce che vale di più di questa skill.** Il documento dell'audit aveva `ok/dbUrl/schemas/findings/summary` e **zero campi di premessa**: un catalogo vuoto produce zero findings su tutte e undici le regole (misurato) e usciva **0, cioè `pass`**, indistinguibile da uno schema pulito. Ora `rls-audit.mjs` calcola `premesse` (tabelle, policy, viste, funzioni security definer, colonne, file pgTAP), esce **2 con il motivo scritto** su zero tabelle, e il dettaglio del passo porta la riga `guardati: N tabelle · N policy · …` **sempre, anche sul verde**. `premesse` entra nel contratto del `--json`: un documento che non la porta non si usa più. Seconda metà: `schemiEsposti(null)` tornava `["public"]` senza errore — il file **assente** non è la chiave assente, e il test che asseriva il contrario è stato **corretto, non tolto**.
  - **M3 — CHIUSO.** Un test pgTAP interamente commentato valeva come un test vero: `-- ` davanti a ogni riga → `block: 0`, identico al test vero. `senzaCommentiSql` toglie **solo** i commenti — il contenuto delle stringhe resta byte per byte, perché un test negativo corretto scrive il tentativo dentro un `throws_ok($$ insert into … $$)` e uno spogliatore che le svuotasse accuserebbe proprio il test scritto bene. Postgres e non un SQL generico: commenti a blocco **annidati**, apici raddoppiati, dollaro. *Limiti dichiarati*: `E'…''` non gestita, e un commento dentro un corpo quotato col dollaro resta dov'è.
  - **M13 — CHIUSO.** Un commento TOML dentro l'array multi-riga produceva schemi fantasma e mandava l'audit in `skipped` accusando un `config.toml` che non è rotto. `senzaCommentoToml` sa distinguere un `#` da un `#` dentro una stringa, e si applica riga per riga **prima** di ogni confronto. *Limite dichiarato*: le stringhe TOML multi-riga (`"""`, `'''`) non sono gestite.
  - **M2 + L2 — CHIUSI.** La URL di connessione esce mascherata da stdout **e** dal `--json`; e la password lascia la riga di comando e passa da `PGPASSWORD`. Il mascheratore è `new URL`, non la regexp che esisteva in casa: misurate su cinque forme, la regexp lascia **metà password in chiaro** su `p%40ss:word`, **distrugge** una URL senza password e **morde dentro la query**.
  - **L12 — CHIUSO**: il difetto ERA l'assenza di rete, e la rete è arrivata (quattro test su `triggerCheNomina`). Con essi, le ultime due `new RegExp` costruite col nome di una colonna passano da `perRegex` — **provate irraggiungibili**, e il test scrive perché.
  - **Due difetti che il referto non aveva**, trovati con le sonde ostili dell'audit degli scanner: `erd-lib.righeDaPsql` non aveva il controllo di arità che `audit-lib` ha guadagnato con H5 (un separatore nel testo dava quattro campi su tre, in silenzio) e `dettaglioAdvisors` prendeva dalla **prima** quadra all'**ultima**, così una quadra nella prosa della CLI faceva sparire «nessun rilievo».
  - **Restano aperti**: il `translate` sulle undici query di testo libero (§H5) — vuole un Postgres vivo, dichiarato nel verbale §7.
...
- **2026-08-07 — il tribunale sul pacchetto stesso.** `/code-inquisition --scope diff` sui 25 file di P.7e: cinque esperti, un critico del roster, **diciannove rilievi**. Referto e misure: `../../PROCESSO-GATE-2-2026-08-06.md` §9. Per questa skill:
  - **`leggiAudit` accettava `premesse.tabelle: 0`**: il controllo di § M12 stava nel PRODUTTORE (`rls-audit.mjs` esce 2), non nel consumatore — e se i due file divergessero il difetto tornerebbe dall'altra parte del contratto. Ora c'e' la **seconda porta**, che e' la stessa forma che la skill sorella ha da sempre per il caso gemello di § H6.
  - **Tre porte ancora aperte sui segreti**, tutte chiuse: `credenzialiPsql` ricadeva sulla URL ORIGINALE quando `decodeURIComponent` sollevava (password di nuovo in `argv`, e psql la rimandava nel proprio stderr che questo gate stampa grezzo); una credenziale in `?password=` sfuggiva a entrambe le funzioni; un `PGPASSWORD` ereditato autenticava quando la URL non ne dichiara.
  - **Due mutazioni sopravvissute alla batteria**: `credenzialiPsql` mutata a no-op restava VERDE — i test esistevano solo in flow-sentinel — e il salto delle stringhe di `chiusuraQuadra` non lo provava nessuno. Entrambe hanno la loro rete.
  - **`primoArrayJson` era quadratico**: 64 000 quadre mai chiuse in **9,2 s**, dentro una funzione che serve solo a riassumere. Ora un tetto di sedici tentativi, e il ripiego sul dump grezzo che esisteva gia'. Misurato: **4 ms**.
  - **Resta aperto**: la E-string con la fuga a barra rovesciata ribalta la parita' degli apici in `senzaCommentiSql` e riapre § M3 da SQL valido (verde falso); `regolaTestNegativi` accetta un attacco che vive dentro una stringa; `primoArrayJson` accetta il primo array che si interpreta, non quello degli advisors.
