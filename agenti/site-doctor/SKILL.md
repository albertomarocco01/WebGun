...
1. **Una voce di conformità ha un proprietario solo, e il proprietario si scrive.** Non «lo guarda qualcuno», non «lo guardano speed-demon e site-doctor»: **uno**. Questa legge non è prudenza, è la cicatrice di un difetto misurato sul progetto pilota: la proprietà dell'Open Graph era assegnata **due volte nello stesso handoff** — a speed-demon *e* a un site-doctor che non esisteva — e la favicon del pilota è stata un `404` su ogni pagina per **tre anelli** (il caso per esteso in `references/perimetro.md` §Il difetto da cui nasce questa skill). Una voce con due proprietari è una voce di nessuno. Una voce che **nessuno** copre si scrive `scoperto`: resta scoperta, ma smette di essere invisibile.
...
2. **Dove un vicino misura, non rimisuro: verifico dichiarato. Dove nessuno guarda, è mio.** Duplicare il controllo di un altro agente significa mantenerne due che prima o poi divergono, e la divergenza si scopre come falso verde. Ma «verifico dichiarato» non vuol dire «mi fido»: vuol dire che il certificato deve **citare il file in cui il vicino l'ha scritto**, quel file deve esistere nel progetto, e deve nominare quella voce. Un rimando a un documento che non c'è è il difetto dell'Open Graph rifatto, con una riga di prosa in più.
   **E «nominata in un documento» non è «guardata da un gate».** Il collaudo P2 ha misurato le nove deleghe leggendo il **codice** dei vicini invece della loro prosa: **sette non reggevano**, favicon e Open Graph comprese — cioè proprio le voci del difetto da cui questa skill è nata. Le deleghe misurate vuote vivono in `SCOPERTE` (`scripts/conformita-lib.mjs`) con accanto il `grep` che le ha trovate, il passo `perimetro` le segnala `issue` a ogni esecuzione, e una riga si toglie da lì solo **rilanciando la misura**, mai leggendo un handoff.
...
| **favicon**: l'icona dichiarata risponde, e se nessuna è dichiarata risponde `/favicon.ico` | **site-doctor** — dal 2026-08-06 (D21) | misurata — passo `favicon`. **È la voce da cui questa skill è nata**: sul pilota è stata un `404` per tre anelli perché due documenti dicevano che se ne occupava qualcun altro, ed era delegata a un gate in cui la parola non compare |
| **Open Graph**: l'anteprima che il sito sceglie, e l'immagine che promette | **site-doctor** — dal 2026-08-06 (D21) | misurata — passo `open-graph` |
| **Dati strutturati** (`application/ld+json`): è JSON valido e dichiara un `@type` | **site-doctor** — dal 2026-08-06 (D21) | misurata — passo `dati-strutturati` |
| **`sitemap.xml`**: risponde, è XML, e i suoi indirizzi sono serviti | **site-doctor** — dal 2026-08-06 (D21) | misurata — passo `sitemap-xml`. È una domanda **diversa** dal leggerla come seconda sorgente della superficie, ed è per questo che è un passo suo |
| **`robots.txt`**: risponde, e **non vieta quello che la sitemap pubblicizza** | **site-doctor** — dal 2026-08-06 (D21) | misurata — passo `robots-txt`. Il confronto fra i due file era elencato fra le cose che **nessun gate della casa** guardava: erano dello stesso agente e nessuno li metteva a confronto |
| **Contrasti di colore** | **speed-demon** — delega **piena dal 2026-08-07** | *verificata dichiarata*, e la dichiarazione ora regge: il grep rilanciato sulla regia a `a1454cf` trova «contrast» in **4 file** di `agenti/speed-demon/` — il suo gate ha il passo `contrasto` che legge `report.audits["color-contrast"]`, non più il solo punteggio di categoria. La riga è uscita da `SCOPERTE` **rilanciando il grep**, come il commento prescriveva: mai leggendo un handoff. I contrasti restano di là perché vogliono un browser, e questo gate non ne apre nessuno |
| `canonical`, `noindex` sulle pagine private | **speed-demon** | *verificata dichiarata* — passo `perimetro`, contro `docs/handoff/<n>-speed-demon.md`. Sono le deleghe che una misura ha trovato **piene**: il suo passo `seo-meta` le legge sull'HTML servito |
| Accessibilità delle rotte amministrative | **gestionale-crafter** — ⚠️ **delega parziale misurata** | *verificata dichiarata*: il suo passo `a11y` lancia `eslint-plugin-jsx-a11y` sui **sorgenti**, non sull'HTML servito dell'area protetta. È la stessa ragione per cui l'accessibilità del sito pubblico è mia; la delega resta perché l'area protetta vuole una sessione, e il gate la segnala `issue` |
...
**Perché l'accessibilità del sito pubblico è mia anche se due vicini la sfiorano.** `vetrina-crafter` linta i **sorgenti** con `jsx-a11y`, e in questa casa è già misurato che il sorgente mente; `speed-demon` misura il **punteggio** Lighthouse su un elenco di pagine **firmato**, e un punteggio non è un elenco di voci da mettere in un certificato. Qui si legge l'HTML servito di **ogni pagina che si raggiunge camminando**: la differenza non è tecnica, è di insieme — i vicini misurano un **campione dichiarato da qualcuno**, questo passo misura la **superficie**. Si vede appena qualcuno aggiunge una pagina: quella entra qui e non entra nell'elenco firmato di nessuno.

**E il limite di questa differenza, misurato invece che promesso:** una pagina che nessuno linka e che non è nella `sitemap.xml` non la trova nemmeno questa camminata. Sul pilota è il caso di `/ordine/<codice>` — pagina vera, esclusa per iscritto dalla misura di speed-demon (`docs/performance.md` §1 S2) e non raggiunta da qui. Resta fuori da entrambi, ed è scritto in §Cosa un gate verde NON prova.

## Gate (`scripts/verify.mjs`) — quattordici passi, id stabili
...
|---|---|---|---|---|---|
| 1 | `certificato` | esiste un certificato **firmato** che dichiara lingue, archiviazione, basi giuridiche e la proprietà delle voci | `docs/conformita.md` | file assente. Firma assente o segnaposto sono un `fail`: sono fatti misurati, non verifiche mancate | mai |
| 2 | `superficie-pubblica` | l'app risponde, è la build **di questo progetto**, e la superficie raggiungibile — camminata da **due sorgenti indipendenti**, riferimenti navigabili (`a`, `area`, `iframe`/`frame`, `form` GET, `meta refresh` — P.6-P5) e `sitemap.xml` (indice risolto) — è quella dichiarata | HTTP dai collegamenti + `sitemap.xml` + `.next/BUILD_ID` | app spenta, `.next/BUILD_ID` assente, camminata interrotta dalla scadenza. Il `BUILD_ID` di **un altro** progetto è un `fail`: è un fatto misurato, non una verifica mancata | mai |
...
| 8 | `favicon` | l'icona **dichiarata** risponde `200`; se nessuna pagina ne dichiara una, risponde `/favicon.ico` | HTML servito + una richiesta per icona | superficie incompleta | mai |
| 9 | `open-graph` | ogni pagina dichiara l'anteprima che vuole, e l'**immagine promessa** risponde | HTML servito + una richiesta per `og:image` | superficie incompleta | mai |
| 10 | `dati-strutturati` | ogni blocco `application/ld+json` **è JSON valido** e dichiara un `@type` | HTML servito | superficie incompleta | mai |
| 11 | `sitemap-xml` | `/sitemap.xml` risponde, **è** una sitemap, e ogni indirizzo che dichiara è servito; una `<sitemapindex>` si riconosce e le sotto-sitemap si seguono (tetto dichiarato: 50, oltre è MANCANTE) | HTTP | superficie incompleta, o indice con più sotto-sitemap del tetto | mai |
| 12 | `robots-txt` | `/robots.txt` risponde e **non vieta ciò che la `sitemap.xml` pubblicizza** | HTTP + la sitemap del passo 11 | superficie incompleta | mai |
| 13 | `perimetro` | ogni voce dell'elenco ha **un solo** proprietario; le delegate citano un file che **esiste** e le **nomina**; le mie riportano l'esito **di questa esecuzione**; le scoperte sono marcate `scoperto` | tabella del certificato + file del progetto + i passi di questo giro | certificato assente. Una tabella presente e illeggibile è un `fail`: è un fatto misurato | mai |
| 14 | `contratto-uscita` | l'handoff esiste e la sua riga `Gate:` dice il vero su **questa** esecuzione | `docs/handoff/` | non si applica: è `pass` o `fail` | mai |

I passi 3-12 leggono l'app **solo** attraverso la superficie che il passo 2 ha stabilito, e **solo se è completa**: una pagina scoperta e non scaricata, una camminata troncata da `--max-pagine` o interrotta dalla scadenza li rende **MANCANTI**, perché ognuno di loro dichiara di guardarle tutte. Se il passo 2 non ha stabilito l'identità dell'app, i passi che la consumano sono **MANCANTI**, non verdi: leggere l'HTML di un'applicazione che non è questa e trovarci l'informativa giusta sarebbe il falso verde più costoso di tutti.
...
| `verify` | **Il gate**: quattordici passi con id stabili, quattro stati, `--json`, `--scadenza` | `scripts/verify.mjs` |
...
- **`verify`** → `node <skill>/scripts/verify.mjs --url <url-della-build> [--json] [--max-pagine N] [--scadenza SECONDI]` dalla radice del progetto generato. Quattordici passi, quattro stati, uscita `0` verde `1` rosso `2` errore. **`--scadenza` ha un default misurato (300 s) e non si può disattivare**: un gate ucciso dal timeout di CI non produce niente, e un silenzio che nessuno ha scritto è il MANCANTE peggiore. I passi non completati sono `skipped` con il conteggio di ciò che avevano guardato — mai `pass`, mai `n/a` — e la riga finale si stampa sempre. `--url` **non ha un default**: senza, il gate legge la riga `URL verificato:` del certificato e altrimenti si rifiuta di indovinare — un gate che indovina `localhost:3000` misura l'app di un altro progetto e stampa `pass`, ed è successo davvero in questa casa il 2026-07-30. All'utente riporto **solo il residuo** e le **verifiche mancanti**.
...
- [ ] **La favicon dichiarata risponde**, l'anteprima Open Graph esiste e la sua immagine risponde, i dati strutturati sono JSON valido (passi `favicon`, `open-graph`, `dati-strutturati`)
- [ ] **La `sitemap.xml` è una sitemap e i suoi indirizzi sono serviti**, e il `robots.txt` non vieta quello che la sitemap pubblicizza (passi `sitemap-xml`, `robots-txt`)
...
Quattordici `pass` dicono una cosa precisa e non di più. Questa sezione esiste perché la sua assenza è il modo in cui un verde diventa una firma in bianco.
...
- **Che un'archiviazione dichiarata «essenziale» lo sia davvero.** Il banner si pretende quando esiste archiviazione **non essenziale**, ma *quale* archiviazione sia essenziale lo scrive chi compila il certificato: un sito che dichiara `essenziale: sì` sul proprio contatore di visite non ha bisogno di banner **secondo questo gate**, e il gate non ha modo di contraddirlo. È il limite di un controllo falsificabile, ed è il motivo per cui quella colonna la firma una persona. Ciò che il gate fa rispettare è più stretto di quanto sembri in una direzione sola: se la **stessa** API di archiviazione è dichiarata da più righe e **una sola** è non essenziale, il banner diventa obbligatorio — un `localStorage` usato per la lingua *e* per un contatore vuole il consenso.
...
- **Che il proprietario dichiarato abbia fatto il suo lavoro.** Il confine si è spostato due volte, e conviene dire dove sta adesso. Il passo `perimetro` prova che la voce è assegnata **a uno solo** e che il file citato **esiste e la nomina**; non legge se quel file dice «fatto» o «da fare», perché quello è prosa e un controllo su prosa libera è un controllo che non c'è. Il collaudo P2 ha aggiunto una misura vera: se il vicino abbia un **passo del gate** che guarda quella voce non è comprensione di un testo — è **codice**, e si legge; le deleghe trovate vuote vivono in `SCOPERTE` col `grep` accanto. D21 ha tratto la conseguenza: **cinque di quelle deleghe non esistono più**, perché la voce è passata a chi la misura. Quello che resta non provato è quindi soltanto questo: che chi ha ancora una delega — speed-demon sui contrasti, gestionale-crafter sull'area protetta — l'abbia poi **eseguita su questa build**. Il gate sa che il passo del vicino esiste (o che non esiste, e allora lo dice); non sa se quel gate è stato lanciato oggi. Quello lo dice la riga «i gate dei vicini sono verdi sulla stessa build» del Gate di chiusura, ed è lavoro dell'agente, non del codice.
  **Una misura che riguarda il vicino porta il commit della regia a cui si riferisce** (`DECISIONI.md` §18 n°3): i gate di questa casa si cambiano sotto a vicenda apposta, e una misura senza il suo commit non è ripetibile.
...
| `references/verifica-deterministica.md` | prima di toccare il gate: i quattordici passi con premessa e MANCANTE, la scadenza complessiva e il suo default misurato, i quattro stati, il contratto `--json`, i falsi verdi possibili, i passi scartati |
