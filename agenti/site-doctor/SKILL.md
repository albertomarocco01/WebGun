...
| `canonical`, `noindex` sulle pagine private | **speed-demon** | *verificata dichiarata* — passo `perimetro`, contro `docs/handoff/<n>-speed-demon.md`. Sono le **due** deleghe che una misura ha trovato piene: il suo passo `seo-meta` le legge sull'HTML servito |
| `sitemap.xml`, `robots.txt` | **speed-demon** — ⚠️ **delega vuota misurata** | li **scrive** e nessun passo del suo gate li rilegge (0 occorrenze di `sitemap`; le occorrenze di `robots` sono tutte `<meta name="robots">`, cioè l'altra voce). Lo dichiara anche il suo `STATO.md` |
| **Open Graph**, **favicon**, dati strutturati | **speed-demon** — ⚠️ **delega vuota misurata** | *verificata dichiarata*. È la voce del difetto: fino al 2026-08-06 la casa la assegnava a due agenti insieme. Ha **un** proprietario e il gate rifiuta la tabella che gliene dà due — ma il collaudo P2 ha misurato che quel proprietario non la guarda (0 occorrenze di `og:`, di `favicon`, di `application/ld` nel suo gate). **La favicon del pilota è stata un `404` per tre anelli, e questa skill la delegava a un gate in cui la parola non compare** |
...
- **Che il proprietario dichiarato abbia fatto il suo lavoro.** Il passo `perimetro` prova che la voce è assegnata **a uno solo** e che il file citato **esiste e la nomina**. Non legge se quel file dice «fatto» o «da fare»: quello è prosa, e un controllo su prosa libera è un controllo che non c'è. Ma il confine è **più stretto di come era scritto**, e il collaudo P2 l'ha misurato: se il vicino abbia un **passo del gate** che guarda quella voce non è comprensione di un testo — è **codice**, e si legge. Le deleghe che una misura ha trovato vuote vivono in `SCOPERTE` col `grep` accanto, e il passo le segnala a ogni esecuzione.
