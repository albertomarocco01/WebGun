...
| **Contrasti di colore** | **speed-demon** — ⚠️ **delega vuota misurata** | *verificata dichiarata*, e la dichiarazione non regge: il 2026-08-06 la parola «contrast» non compare in **nessun file** di `agenti/speed-demon/`. Il suo gate esegue Lighthouse con la categoria `accessibility`, che l'audit `color-contrast` lo contiene, ma legge **solo il punteggio** della categoria e mai il singolo audit. Misurarli qui vorrebbe comunque dire risolvere cascata e specificità senza browser: la voce resta di là e il gate la segnala `issue` a ogni giro |
...
| 2 | `superficie-pubblica` | l'app risponde, è la build **di questo progetto**, e la superficie raggiungibile — camminata da **due sorgenti indipendenti**, collegamenti e `sitemap.xml` — è quella dichiarata | HTTP dai collegamenti + `sitemap.xml` + `.next/BUILD_ID` | app spenta, `.next/BUILD_ID` assente, camminata interrotta dalla scadenza. Il `BUILD_ID` di **un altro** progetto è un `fail`: è un fatto misurato, non una verifica mancata | mai |
...
| 11 | `sitemap-xml` | `/sitemap.xml` risponde, **è** una sitemap, e ogni indirizzo che dichiara è servito | HTTP | superficie incompleta | mai |
