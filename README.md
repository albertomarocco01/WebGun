...
16. 🟢\*\*\*\*\*\*\* **Launchpad**
    L'ultimo miglio: dal codice al sito online, su Vercel o Cloudflare, con DNS, domini e certificati SSL. Ma non è un deployer con un cancello davanti: **è il cancello**, e il deploy è ciò che succede quando si apre. Quattro leggi: non si pubblica su gate rosso — di *nessuno* degli agenti a monte, ed è una condizione che flow-sentinel e speed-demon avevano posto prima che questo agente esistesse; la pubblicazione la autorizza un umano e **la conferma è sul contenuto, non sul comando** (`DECISIONI.md` §6); nel pacchetto che parte non viaggia nessun segreto — né in HEAD né nella storia — e la chiave `service_role` non entra mai in un progetto generato; e ciò che va online deve restare **dimostrabile dopo**, con un'impronta *derivata dal commit* invece che registrata, perché il provider ricostruisce dal sorgente e un `BUILD_ID` casuale non prova niente un minuto dopo. Gate a nove passi. Manuale: `agenti/launchpad/SKILL.md`.
17. 🟢 **DemonIAc**
...
\*\*\*\*\*\*\* **launchpad è 🟢 come strumento, e non ha mai pubblicato niente.** Progettato e costruito il 2026-08-06 in un solo pacchetto (P.5), col gate scritto **prima** del flusso: nove passi, **105 test** sugli script, quattro reference, due template. Il gemello pulito chiude **VERDE 9/9**; **36 classi di sabotaggio, 36 rosse, zero non prese**. Sul pilota `fornodoro` — un sito completo con cinque gate verdi che **non si deve pubblicare** — esce **ROSSO**, e i motivi che stampa li ha trovati lui nel registro del debito: n°4, n°12, n°17, n°27, n°32, di cui il n°12 non era nemmeno nell'elenco che il mandato gli aveva dato. Due di quei motivi li **rimisura da solo** invece di leggerli, ed è la prova che il verbale esisteva per produrre: quando P.4g ha chiuso il n°32, il passo `runtime-riproducibile` è passato da **1 bloccante a 0** senza che una riga di questa skill cambiasse. La sosta di metà pacchetto ha trovato **sei** passi che potevano essere verdi su un deploy da non fare, e uno era un difetto già scritto: `generateBuildId` con lo SHA come **letterale**, che al commit successivo avrebbe dichiarato con sicurezza il commit sbagliato. E un tribunale a tre periti (`/code-inquisition`, modelli diversi per de-correlare gli errori) ha trovato **32 rilievi** — tutti chiusi, ognuno col suo test di regressione — di cui il più grave era che **il rimedio che questa skill prescrive rompeva la build del cliente**: il frammento `generateBuildId` usa `require`, che in un `next.config.mjs` non esiste, verificato con una build Next.js 16.3.0 vera. Due periti su tre, con mandati e riproduzioni diversi, hanno confermato **lo stesso difetto**: un file salvato in **UTF-16** — il default di `Out-File` su Windows — veniva trattato come binario e mai guardato, e un `.env` tracciato con dentro una chiave `service_role` faceva uscire il gate **verde**. E la riga che vale per tutta la casa, per la sesta volta: **gli strumenti statici erano tutti verdi** — ESLint, knip, jscpd, 87 test — e non hanno visto nessuno dei 32. Resta **non usabile su un progetto cliente** per due motivi separati: **nessun deploy è mai stato eseguito** — non un account, non un dominio, non un centesimo, ed era una condizione esplicita del mandato — e il collaudo avversario (P2) non è stato fatto. Verbale: `agenti/launchpad/COSTRUZIONE-2026-08-06.md`; l'elenco di ciò che solo un umano potrà provare è in `agenti/launchpad/STATO.md` §Cosa non è mai stato provato.

\*\*\*\*\*\* **site-doctor è 🟢 come strumento, e ha avuto il suo collaudo avversario.** Progettata e costruita il 2026-08-06 in **un solo pacchetto** (P0+P1 uniti, decisione D17 del cantiere), poi collaudata (P2) e ricollaudata (P3) nella notte fra il 6 e il 7. Il gate ha **quattordici passi** e **quattro stati** — `NON APPLICABILE` esiste e costa una premessa misurata e stampata — e chiude **VERDE 14/14** sul banco «studio legale» bilingue, **ROSSO su 32 classi di sabotaggio su 42** (le altre dieci sono limiti dichiarati che reggono, o assenze che per contratto valgono un rilievo e non un bloccante). La costruzione era uscita ROSSA sul pilota per cinque motivi che nessuno dei cinque gate esistenti vede. Poi ogni giro ha trovato quello che il precedente non poteva: il **tribunale** della costruzione trentatré rilievi (il più grave apriva tutti i passi insieme — in HTML un `<!--` dentro il valore di un attributo è testo, quindi due `<div>` invisibili facevano sparire immagini senza `alt`, campi che raccolgono dati personali e terzi non dichiarati); il **collaudo P2** quattordici difetti eseguendo invece di leggere, fra cui che **sette deleghe su nove erano vuote, favicon compresa** — la skill nata dalla favicon a `404` la delegava a un gate in cui quella parola non compare; il **tribunale di P3**, sei periti su `scripts/`, **quarantotto rilievi con ESLint, knip, jscpd e la batteria tutti verdi**: una chiave universale da un carattere (un `>` dentro un valore di attributo quotato apriva quattro passi insieme), quattro quadratiche distinte e un crash deterministico, una junction NTFS dentro `docs/` che faceva leggere fuori dal progetto, e un refuso nell'intestazione della tabella che spegneva in silenzio la regola per cui questa skill esiste. **D21** ha poi riportato a casa cinque voci — favicon, Open Graph, JSON-LD, `sitemap.xml`, `robots.txt` — perché la proprietà segue la misura e non l'argomento: le voci scoperte passano da otto a tre. Batteria **264 test**. Il gate finisce **sempre con un verdetto**: `--scadenza` ha un default di 300 s estrapolato da una pendenza misurata, e i passi non completati sono `skipped` con il conteggio di quello che avevano guardato. Verbali: `agenti/site-doctor/COSTRUZIONE-2026-08-06.md`, `COLLAUDO-2026-08-06.md`, `P6-P3-2026-08-06.md`.
